<template>
  <div class="bg-white bg-opacity-5 md:py-20 py-14">
    <Container>
      <div class="grid lg:grid-cols-6 md:grid-cols-4 sm:grid-cols-3 grid-cols-1 sm:gap-0 gap-3 md:px-0 px-6 ">
        <div class="sm:col-span-2 col-span-1">
          <img src="@/assets/img/header/logo.svg" alt="logo" class="mt-2 mb-4">
          <div class="sm:w-1/2 w-8/12 text-sm font-thin text-white leading-5 italic">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit.
          </div>
        </div>
        <div class=" text-white sm:mt-0 mt-8">
          <h2 class="mb-4 italic font-medium leading-5">
            Join the Community
          </h2>
          <div class="flex gap-3">
            <img src="@/assets/img/footer/discord.svg" alt="dicord">
            <img src="@/assets/img/footer/twitter.svg" alt="twitter">
            <img src="@/assets/img/footer/instagram.svg" alt="instagram">
          </div>
        </div>
        <div class=" text-white md:mt-0 mt-8">
          <div class=" w-max md:mx-auto">
            <h2 class="mb-4 italic font-medium ">
              Marketplace
            </h2>
            <ul class="flex flex-col gap-3 text-sm font-thin italic">
              <li>Collections</li>
              <li>Stats</li>
            </ul>
          </div>
        </div>
        <div class=" text-white lg:mt-0 mt-8">
          <div class="w-max lg:mx-auto">
            <h2 class="mb-4 italic font-medium ">
              Account
            </h2>
            <ul class="flex flex-col gap-3 text-sm font-thin italic">
              <li>Profile/Settings</li>
              <li>Collected Items</li>
              <li>Favorites</li>
              <li>Followed Collections</li>
              <li>Activity</li>
              <li>Harmonie Rewards</li>
            </ul>
          </div>
        </div>
        <div class=" text-white lg:mt-0 mt-8">
          <div class=" w-max lg:mx-auto">
            <h2 class="mb-4 italic font-medium ">
              Company
            </h2>
            <ul class="flex flex-col gap-3 text-sm font-thin italic">
              <li>About Us</li>
              <li>Privacy Policy</li>
              <li>Cookie Policy</li>
              <li>Terms &amp; Conditions</li>
              <li>Contact</li>
            </ul>
          </div>
        </div>
      </div>
    </Container>
  </div>
</template>
<script>
export default {
  data: () => ({

  })
}
</script>
